   ...
   public class SurfaceTile
    {
        ...
        public SurfaceTile(double north, double south, double west, double east, int level, WorldSurfaceRenderer parentWorldSurfaceRenderer)
        {
            ...
            m_BoundingBox = new BoundingBox(south, north, west, east, World.EquatorialRadius, World.EquatorialRadius * scale);
        }
        ...
        public void Update(DrawArgs drawArgs)
        {
            try
            {
                ...
                if (drawArgs.WorldCamera.TrueViewRange < Angle.FromDegrees(3.0f * tileSize)
                        && SMath.SphericalDistance(Angle.FromDegrees(centerLatitude), Angle.FromDegrees(centerLongitude),
        ...
        public bool IsRenderable(DrawArgs drawArgs)
        {
            ...
            if (!m_Initialized ||
                ...
                SMath.SphericalDistance(Angle.FromDegrees(_centerLat), Angle.FromDegrees(_centerLon),
            ...
            using (Surface renderSurface = m_RenderTexture.GetSurfaceLevel(0))
            {
                ...
                lock (m_ParentWorldSurfaceRenderer.SurfaceImages.SyncRoot)
                {
                    for (int i = 0; i < m_ParentWorldSurfaceRenderer.SurfaceImages.Count; i++)
                    {
                        ...
                        drawArgs.device.SamplerState[0].AddressU = TextureAddress.Border;
                        drawArgs.device.SamplerState[0].AddressV = TextureAddress.Border;
        ...
        public void Render(DrawArgs drawArgs)
        {
            try
            {
                ...
                int m_IndicesElevatedLength = WorldSurfaceRenderer.m_IndicesElevated.Length;
                ...
                if (!nwRendered && m_DynamicTexture.nwVerts != null)
                {
                    drawArgs.device.DrawIndexedUserPrimitives(
                        ...
                        (m_NwIndices != null ? m_NwIndices.Length / 3 : m_IndicesElevatedLength / 3),
                        (m_NwIndices != null ? m_NwIndices : WorldSurfaceRenderer.m_IndicesElevated),
                        true,
                ...
                if (!neRendered && m_DynamicTexture.neVerts != null)
                {
                    drawArgs.device.DrawIndexedUserPrimitives(
                        ...
                        (m_NeIndices != null ? m_NeIndices.Length / 3 : m_IndicesElevatedLength / 3),
                        (m_NeIndices != null ? m_NeIndices : WorldSurfaceRenderer.m_IndicesElevated),
                        true,
                ...
                if (!swRendered && m_DynamicTexture.swVerts != null)
                {
                    drawArgs.device.DrawIndexedUserPrimitives(
                        ...
                        0,
                        m_DynamicTexture.nwVerts.Length,
                        (m_SwIndices != null ? m_SwIndices.Length / 3 : m_IndicesElevatedLength / 3),
                        (m_SwIndices != null ? m_SwIndices : WorldSurfaceRenderer.m_IndicesElevated),
                        true,
                ...
                if (!seRendered && m_DynamicTexture.seVerts != null)
                {
                    drawArgs.device.DrawIndexedUserPrimitives(
                        ...
                        (m_SeIndices != null ? m_SeIndices.Length / 3 : m_IndicesElevatedLength / 3),
                        (m_SeIndices != null ? m_SeIndices : WorldSurfaceRenderer.m_IndicesElevated),
                        true,
                ...
                for (int i = 0; i < thisVertexDensityElevatedPlus3; i++)
                {
                    ...
                    for (int j = 0; j < thisVertexDensityElevatedPlus3; j++)
                    {
                        ...
                        if (i == 0 || j == 0 || i == thisVertexDensityElevatedPlus3 - 1 || j == thisVertexDensityElevatedPlus3 - 1)
                        {
                            ...
                            Vector3 v = SMath.SphericalToCartesian(
                                latitude,
                                longitude,
                                World.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
                        ...
                        else
                        {
                            ...
                            Vector3 v = SMath.SphericalToCartesian(
                                m_North - scaleFactor * latrange * (i - 1),
                                m_West + scaleFactor * lonrange * (j - 1),
                                World.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
                ...
                for (int i = 0; i < thisVertexDensityElevatedPlus3; i++)
                {
                    ...
                    for (int j = 0; j < thisVertexDensityElevatedPlus3; j++)
                    {
                        ...
                        else
                        {
                            ...
                            Vector3 v = SMath.SphericalToCartesian(
                                m_North - scaleFactor * latrange * (i - 1),
                                0.5f * (m_West + m_East) + (float)scaleFactor * lonrange * (j - 1),
                                World.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
                ...
                for (int i = 0; i < thisVertexDensityElevatedPlus3; i++)
                {
                    ...
                    for (int j = 0; j < thisVertexDensityElevatedPlus3; j++)
                    {
                        ...
                        else
                        {
                            ...
                            Vector3 v = SMath.SphericalToCartesian(
                                0.5 * (m_North + m_South) - scaleFactor * latrange * (i - 1),
                                m_West + (float)scaleFactor * lonrange * (j - 1),
                                World.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
                ...
                for (int i = 0; i < thisVertexDensityElevatedPlus3; i++)
                {
                    ...
                    for (int j = 0; j < thisVertexDensityElevatedPlus3; j++)
                    {
                        ...
                        else
                        {
                            ...
                            Vector3 v = SMath.SphericalToCartesian(
                                0.5 * (m_North + m_South) - scaleFactor * latrange * (i - 1),
                                0.5 * (m_West + m_East) + (float)scaleFactor * lonrange * (j - 1),
                                World.EquatorialRadius + m_ParentWorldSurfaceRenderer.DistanceAboveSeaLevel + height);
        ...
        private void calculate_normals(ref CustomVertex.PositionNormalTextured[] vertices, short[] indices)
        {
            ...
            for (int i = 0; i < m_IndicesElevatedLength; i += 3)
            {
        ...
        /// </summary>
        /// <value></value>        
        public Texture RenderTexture { get { return m_RenderTexture; } }
    }
}