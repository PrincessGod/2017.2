...
namespace R.Earth.QuadTile
{
    ...
    /// </summary>
    public class ImageAccessor : IImageAccessor, IDisposable
    {
        #region Private Members
        protected DownloadQueue m_downloadQueue = new DownloadQueue();
        ...
        public virtual ImageTileInfo GetImageTileInfo(int level, int row, int col, bool allowCache)
        {
            ...
            string relativePath = String.Format(@"{0}\{1:f0}\{1:f0}_{2:f0}.{4}", level, row, col, m_textureSizePixels, m_imageFileExtension);
            ...
            if (m_imageTileService != null)
            {
                //return new ImageTileInfo(m_DefautlimageFile);
                return new ImageTileInfo(cacheFullPath, m_imageTileService.GetImageTileServiceUri(level, row, col));
            }
        ...
        string m_DefautlimageFile = "GreenMatrixCode.jpg";
