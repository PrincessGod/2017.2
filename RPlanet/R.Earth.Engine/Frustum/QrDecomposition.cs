...
{
    /// <summary>
    ///	  QR decomposition for a rectangular MatrixV.
    /// </summary>
    /// <remarks>
    ///   For an m-by-n MatrixV <c>A</c> with <c>m &gt;= n</c>, the QR decomposition is an m-by-n
    ///   orthogonal MatrixV <c>Q</c> and an n-by-n upper triangular 
    ///   MatrixV <c>R</c> so that <c>A = Q * R</c>.
    ///   The QR decompostion always exists, even if the MatrixV does not have
    ...
    public class QrDecomposition
    {
        ...
        /// <summary>Shows if the MatrixV <c>A</c> is of full rank.</summary>
        ...
        public MatrixV UpperTriangularFactor
        {
            get
            {
                int n = this.QR.Columns;
                MatrixV X = new MatrixV(n, n);
                double[][] x = X.Array;
                double[][] qr = QR.Array;
                for (int i = 0; i < n; i++)
                {
        ...
        public MatrixV OrthogonalFactor
        {
            get
            {
                MatrixV X = new MatrixV(QR.Rows, QR.Columns);
                double[][] x = X.Array;
                double[][] qr = QR.Array;
