    ...
    public sealed class ImageHelper
    {
        ...
        public static Texture LoadTexture(string textureFileName, int transparentRangeDarkColor, int transparentRangeBrightColor)
        {
            ...
            catch (OutOfMemoryException)
            {
                ...
                Texture tex = new Texture(DrawArgs.Instance.device, fs, Usage.None, Pool.Managed);
                fs.Close();
                return tex;
            }
            ...
            {
                return new Texture(DrawArgs.Instance.device, transparentImage, Usage.None, Pool.Managed);
            }
        }
        ...
        public static Texture LoadTexture(Stream textureStream, int colorKey, Format textureFormat)
        {
            try
            {
                Texture texture = TextureLoader.FromStream(DrawArgs.Instance.device, textureStream, 0, 0,
            ...
            try
            {
                ...
                using (Bitmap image = (Bitmap)Image.FromStream(textureStream))
                {
                    Texture texture = new Texture(DrawArgs.Instance.device, image, Usage.None, Pool.Managed);
                    return texture;
                }
            }
            catch
        ...
        public static Texture LoadIconTexture(string relativePath)
        {
            try
            {
                ...
                if (File.Exists(fullPath))
                    return TextureLoader.FromFile(DrawArgs.Instance.device, fullPath, 0, 0, 1, Usage.None,
            ...
            using (Bitmap bitmap = CreateDefaultImage())
                return new Texture(DrawArgs.Instance.device, bitmap, 0, Pool.Managed);
        }
